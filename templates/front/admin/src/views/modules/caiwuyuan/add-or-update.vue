...
export default {
  data() {
	...
	shouji : false,
      ...
      rules: {
          caiwugonghao: [
                { required: true, message: '财务工号不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `caiwuyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.caiwuyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
