<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="资产类型" prop="zichanleixing">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="资产状态" prop="zichanzhuangtai">
          <el-select :disabled="ro.zichanzhuangtai" v-model="ruleForm.zichanzhuangtai" placeholder="请选择资产状态">
            <el-option
                v-for="(item,index) in zichanzhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="资产状态" prop="zichanzhuangtai">
	      <el-input v-model="ruleForm.zichanzhuangtai"
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="资产说明" prop="zichanshuoming">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="资产说明"
                  v-model="ruleForm.zichanshuoming" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zichanshuoming" label="资产说明" prop="zichanshuoming">
                    <span>{{ruleForm.zichanshuoming}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	zichantupian : false,
	zichanpinpai : false,
	zichanzhuangtai : false,
	zichanshuliang : false,
	zichanshuoming : false,
      },
      ruleForm: {
        zichanmingcheng: '',
        zichanleixing: '',
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zichanpinpai = true;
          ...
          if(o=='zichanzhuangtai'){
            this.ruleForm.zichanzhuangtai = obj[o];
	    this.ro.zichanzhuangtai = true;
            continue;
          }
          if(o=='zichanshuliang'){
	    ...
	    this.ro.zichanshuliang = true;
          ...
          if(o=='zichanshuoming'){
            this.ruleForm.zichanshuoming = obj[o];
	    this.ro.zichanshuoming = true;
            continue;
          }
        }
      }
      ...
      this.$http({
		...
		if(this.$storage.get("role")!="管理员") {
          ...
          this.$message.error(data.msg);
            ...
            this.zichanzhuangtaiOptions = "未使用,已使用,维修中,已报废".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `zichanxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zichanxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zichanxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
