<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="校级审核" prop="xiaojishenhe">
          <el-select :disabled="ro.xiaojishenhe" v-model="ruleForm.xiaojishenhe" placeholder="请选择校级审核">
            <el-option
                v-for="(item,index) in xiaojishenheOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="校级审核" prop="xiaojishenhe">
	      <el-input v-model="ruleForm.xiaojishenhe"
                placeholder="校级审核" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  data() {
	...
	shhf : false,
      ...
      rules: {
          ...
          baoxiushuliang: [
                { required: true, message: '报修数量不能为空', trigger: 'blur' },
                { validator: validateIntNumber, trigger: 'blur' },
          ],
          baoxiushuoming: [
          ],
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.baoxiushuoming = true;
          ...
          if(o=='xiaojishenhe'){
            this.ruleForm.xiaojishenhe = obj[o];
	    this.ro.xiaojishenhe = true;
            continue;
          }
          if(o=='baoxiuriqi'){
      ...
      this.$http({
		...
		if(json.lianxidianhua!=''&&json.lianxidianhua){
          ...
          this.$message.error(data.msg);
            ...
            this.xiaojishenheOptions = "已审核,未审核".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
