<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="报修数量" prop="baoxiushuliang">
          <el-input v-model="ruleForm.baoxiushuliang" 
              placeholder="报修数量" clearable  :readonly="ro.baoxiushuliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="报修数量" prop="baoxiushuliang">
              <el-input v-model="ruleForm.baoxiushuliang" 
                placeholder="报修数量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="复审日期" prop="fushenriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.fushenriqi" 
                type="date"
                :readonly="ro.fushenriqi"
                placeholder="复审日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fushenriqi" label="复审日期" prop="fushenriqi">
              <el-input v-model="ruleForm.fushenriqi" 
                placeholder="复审日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="报修说明" prop="baoxiushuoming">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="报修说明"
                  v-model="ruleForm.baoxiushuoming" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.baoxiushuoming" label="报修说明" prop="baoxiushuoming">
                    <span>{{ruleForm.baoxiushuoming}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	baoxiushuliang : false,
	baoxiushuoming : false,
	fushenriqi : false,
	yonghuming : false,
	bumen : false,
	lianxidianhua : false,
  ...
  created() {
	this.ruleForm.fushenriqi = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zichanshuliang = true;
          ...
          if(o=='baoxiushuliang'){
            this.ruleForm.baoxiushuliang = obj[o];
	    this.ro.baoxiushuliang = true;
            continue;
          }
          if(o=='baoxiushuoming'){
            this.ruleForm.baoxiushuoming = obj[o];
	    this.ro.baoxiushuoming = true;
            continue;
          }
          if(o=='fushenriqi'){
            this.ruleForm.fushenriqi = obj[o];
	    this.ro.fushenriqi = true;
            continue;
          }
          if(o=='yonghuming'){
    ...
    onSubmit() {
      ...
      if(parseInt(this.ruleForm.baoxiushuliang)>parseInt(this.ruleForm.zichanshuliang)){
        this.$message.error("报修数量不能超过资产数量");
        return
      }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `weixiufushen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.weixiufushenCrossAddOrUpdateFlag = false;
						 this.parent.search();
